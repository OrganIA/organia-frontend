  ...
  <div>
    <form class="form-control" @submit.prevent="submitForm">
      <div class="form-fields">
        ...
        <input v-model="name" type="text" class="input mb-6" required/>
        <div class="form-input small">
          <label class="label">Ville</label>
          <input v-model="city_name" type="text" class="input mb-6" required/>
        </div>
        <div class="form-input small">
          <label class="label">Code de département</label>
          <input v-model="department_code" type="text" class="input mb-6"  required/>
        </div>
      </div>
  ...
  methods: {
    submitForm() {
        this.$http
          ...
          .then(() => {
            this.$toast.success("Creation de l'hopitale réussi !");
